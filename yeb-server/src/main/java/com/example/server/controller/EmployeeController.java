...
public class EmployeeController {
    ...
    public List<PoliticsStatus> getAllPoliticsStatus(){
        //sql:  SELECT id,name FROM t_politics_status
    ...
    public List<Joblevel> getAlljoblevel(){
        //sql:   SELECT id,name,titleLevel,createDate,enabled FROM t_joblevel
    ...
    public List<Nation> getAllNation(){
        //sql:  SELECT id,name FROM t_nation
    ...
    public List<Position> getAllPosition(){
        //sql:  SELECT id,name,createDate,enabled FROM t_position
    ...
    public RespBean updateEmp(@RequestBody Employee employee){
        if (employeeService.updateById(employee)){
            /*sql:   UPDATE t_employee SET name=?, gender=?, birthday=?, idCard=?, wedlock=?, nationId=?,
                        nativePlace=?, politicId=?, email=?, phone=?, address=?, departmentId=?, jobLevelId=?, posId=?, engageForm=?,
                        tiptopDegree=?, specialty=?, school=?, beginDate=?, workState=?, workID=?, conversionTime=?, beginContract=?,
                        endContract=?, salaryId=? WHERE id=?*/
    ...
    public RespBean deleteEmp(@PathVariable Integer id){
        if (employeeService.removeById(id)){
            // sql:  DELETE FROM t_employee WHERE id=?
    ...
    public void exportEmployee(HttpServletResponse response){
        ...
        //导出工具类  ExcelExportUtil.exportExcel（设置的参数，需要导出的对象，导出的数据）
    ...
    public RespBean importEmployee(MultipartFile file){
        ...
        try {
            ...
            list.forEach(employee ->{
                ...
                // System.out.println(nations.get(nations.indexOf(new Nation(employee.getNation().getName()))));
